...
function checkForm(a,b){}function test(a){if(a==0){$("iframe-container").setStyle("height",27)}else if(a==1){$("iframe-container").setStyle("height",397)}var b=0}var Ubisoft={settings:{flash:{version:9,rev:0,build:48},hasFlash:null,debug:{Surrounder:false,Promo:false},stopFirefox2Animation:false,transition:{duration:"350"}},Global:null,Navigation:null,Login:null,Localization:null,Promo:null,Homepage:null,Gamefinder:null,GameListing:null,Advertisement:null};DropdownManager={select:function(a){this.getElement("span").set("html",a.innerHTML);$$("select[id$="+this.get("label")+"]")[0].value=a.get("label");this.fireEvent("click")}};Ubisoft.Global={init:function(){if($("btn-submit-container")){$("btn-submit-container").addEvent("mouseenter",function(){this.getElement("a").addClass("hover")});$("btn-submit-container").addEvent("mouseleave",function(){this.getElement("a").removeClass("hover")})}$$("input.submit").each(function(a){a.addEvent("mouseenter",function(){this.addClass("over")});a.addEvent("mouseleave",function(){this.removeClass("over")})});if(BrowserObject.Engine.trident4){$$("a.ie6").each(function(a){a.addEvent("mouseenter",function(){this.addClass("hover")});a.addEvent("mouseleave",function(){this.removeClass("hover")})})}$$("input.text").each(function(a){if(!a.get("label")){a.set("label",a.value)}a.addEvent("focus",function(){if(a.value==a.get("label")){a.value=""}});a.addEvent("blur",function(){if(a.value.trim().length==0){}})});var a=20;$$("div.dropdown").each(function(b,c){b.setStyle("z-index",a-c);var d=b.getElement("div.nav-wrapper");if(d){if(b.get("label")!=null){index=0;var e=DropdownManager.select.bind(b);d.getElements("a").each(function(a,b){a.addEvent("click",function(){e(a);return false});index++})}b.addEvent("click",function(){var a=this.getElement("div.nav-wrapper");if(a.getStyle("visibility")=="visible"){var b=Ubisoft.Navigation.closeItem.bind(a);b();return}Surrounder.hide();a.setStyles({height:a.getChildren()[0].getSize().y,visibility:"visible"});Ubisoft.Navigation.element=a});b.addEvent("mouseenter",function(){this.addClass("hover");var a=Ubisoft.Navigation.closeItem.bind(this.getElement("div.nav-wrapper"));a()});b.addEvent("mouseleave",function(){this.removeClass("hover");var a=Ubisoft.Navigation.closeItem.bind(this.getElement("div.nav-wrapper"));a()})}else{b.addEvent("mouseenter",function(){Surrounder.hide()})}})}};Ubisoft.Navigation={element:null,settings:{transition:{duration:"200"},offset:39},init:function(){var a=Ubisoft.Homepage&&BrowserObject.Engine.gecko&&BrowserObject.Engine.version==18;if(Ubisoft.settings.stopFirefox2Animation&&BrowserObject.Engine.gecko&&BrowserObject.Engine.version==18)this.settings.transition.duration="0";$$("li.nav").each(function(a,b){var c=a.getElement("div.nav-wrapper");if(c||a.hasClass("dropmenu")){var d=-1;c.getElements("a").each(function(a){d=a.getSize().x>d?a.getSize().x:d;a.setStyle("display","block")});d=d==-1?-1:d<90?90:d;if(d!=-1&&a.hasClass("dropmenu")){c.setStyle("width",d+15)}if(navigator.platform&&navigator.platform=="MacPPC"&&BrowserObject.Engine.gecko19){c.setStyles({background:"black"})}a.addEvent("mouseenter",function(){if(Ubisoft.Localization.isOpen)return;Surrounder.hide();var a=this.getElement("div.nav-wrapper");a.setStyles({height:a.getChildren()[0].getStyle("height"),visibility:"visible"});Ubisoft.Navigation.element=a;this.addClass("on");this.addClass("white")});a.addEvent("mouseleave",function(){var a=Ubisoft.Navigation.closeItem.bind(this.getElement("div.nav-wrapper"));a()});if(BrowserObject.Engine.webkit){var e=a.getElement("img.arrow");if(e)e.setStyle("left",a.getSize().x-(a.id=="locale"?10:45))}}else{a.addEvent("mouseenter",function(){if(Ubisoft.Localization.isOpen)return;Surrounder.hide()})}})},closeItem:function(){this.setStyles({height:0,visibility:"hidden"});this.getParent().removeClass("on");this.getParent().removeClass("white")}};Ubisoft.Localization={searchBar:null,settings:{transition:{duration:"350"},offset:25},isOpen:false,list:null,subList:null,init:function(){},setLocale:function(a){$("internaltionalselector").value=a.name;Surrounder.hide();$("Default").submit()}};var Surrounder={elements:{parent:null,left:null,top:null,right:null,bottom:null},callback:null,initialize:function(){if(Ubisoft.settings.debug.Surrounder){new Asset.css("resources/css/debug_stylesheet.css",{id:"debugCSS"})}if(this.elements.parent){this.elements.parent=$("surround");this.elements.top=$("surround-top");this.elements.bottom=$("surround-bottom");this.elements.left=$("surround-left");this.elements.right=$("surround-right");this.elements.parent.addEvent("mouseover",this.hide)}},el:null,draw:function(a,b,c){if(!c)c=0;this.callback=b;this.el=a;var d=a.getCoordinates();var e=window.getScrollSize();var f=window.getSize();f.x=f.x-25;Surrounder.elements.top.setStyles({height:d.top-c,width:f.x});Surrounder.elements.bottom.setStyles({height:e.y-(d.top+d.height),top:d.top+d.height,width:f.x});Surrounder.elements.left.setStyles({height:d.height+c,top:d.top-c,width:d.left.limit(0,99999)});Surrounder.elements.right.setStyles({height:d.height+c,top:d.top-c,width:(f.x-(d.width+d.left)).limit(0,99999),left:d.width+d.left})},hide:function(){return;Surrounder.elements.parent.setStyle("display","none");if(Surrounder.callback){Surrounder.callback(Surrounder.el)}}};Ubisoft.Advertisement={init:function(){if($("promo-spot")){var a=$("promo-spot").get("html");if(a.length>=5){$("promo-advertisement").setStyle("visibility","visible")}}}};window.addEvent("domready",function(){Surrounder.initialize();Ubisoft.Global.init();Ubisoft.Navigation.init();Ubisoft.Advertisement.init()})
function rand(a){return Math.floor(Math.random()*a+1)}function hideFlashPromo(){Ubisoft.Homepage.loadGamefinder()}function trim(a){return a.replace(/^\s+|\s+$/g,"")}function openWindow(a,b){var c=window.open(a,b);if(!c){Ubisoft.Gamefinder.openWindowFromSwf(a,b)}else{c.focus()}}var GamefinderParams={keyword:"",featured:"1",genre:"0",platform:"0",rating:"0",sort:"ReleaseDate_DESC"};Ubisoft.Homepage={Cookie:{getValue:function(){if(Cookie.read("gamefinderagegate")){return Cookie.read("gamefinderagegate")}else{return-1}},setValue:function(a){Cookie.write("gamefinderagegate",a)}},init:function(){$$("a.anim-link").each(function(a){a.addEvent("mouseenter",function(){this.getElement("span").addClass("on");this.getElement("span").removeClass("off")});a.addEvent("mouseleave",function(){this.getElement("span").addClass("off");this.getElement("span").removeClass("on")})});Ubisoft.settings.hasFlash=FlashDetect.DetectVersion(Ubisoft.settings.flash.version,Ubisoft.settings.flash.rev,Ubisoft.settings.flash.build);if(!Ubisoft.settings.hasFlash){$("flash-container").addClass("noflash");return}this.openPromotion();return},openPromotion:function(){Ubisoft.Homepage.initFlashArea();$("game-finder").setStyles({width:"100%"});var a=document.getElementById("locale").firstChild.nodeValue.substr(0,2).toLowerCase();var b=trim(document.getElementById("locale").firstChild.nodeValue.toLowerCase());if(trim(jQuery("#dvTakeover").html()).length>0){jQuery("#game-finder-flash").html(jQuery("#dvTakeover").html())}else{hideFlashPromo()}},closePromotion:function(){dialog.hide();Ubisoft.Homepage.loadGamefinder()},initFlashArea:function(){if($("flash-container").getStyle("height")=="0px"){$("flash-container").setStyles({"border-bottom":"none",height:"auto"});$("game-finder").setStyles({height:446,width:996})}$("primary-spotlight").setStyle("display","none")},loadGamefinder:function(){Ubisoft.Homepage.initFlashArea();$("game-finder").setStyles({width:996});var a={configUrl:"http://cdn.webperflab.com/2012/grayfox/a4/www.ubi.com/config/application.07312012.xml",configUrlMain:"http://cdn.webperflab.com/2012/grayfox/a4/www.ubi.com/config/mainApplication.07312012.xml",search:escape(JSON.encode(GamefinderParams))};var b={};var c={wmode:"opaque",align:"middle",scale:"noscale",allowFullScreen:"false",allowScriptAccess:"sameDomain",play:"true",quality:"high"};swfobject.embedSWF("http://cdn.webperflab.com/2012/grayfox/a4/www.ubi.com/resources/assets/flash/main.swf?"+rand(1e6),"game-finder-flash",996,441,"9.0.0",null,a,c,b)}};window.addEvent("domready",function(){Ubisoft.Homepage.init()});
function ubi_check_screen(){if(!self.screen)return"";var a="hres="+escape(screen.width)+"&vres="+escape(screen.height)+"&ahres="+escape(screen.availWidth)+"&avres="+escape(screen.availHeight)+"&depth="+escape(screen.colorDepth);if(screen.bufferDepth)a+="&bufferDepth="+escape(screen.bufferDepth);return a}function ubi_check_language(){if(self.navigator.userLanguage)return"userLang="+escape(navigator.userLanguage)+"&sysLang="+escape(navigator.systemLanguage)+"&browserLang="+escape(navigator.browserLanguage);return""}function ubi_check_referer(){if(document.referrer)return encodeURIComponent(document.referrer);return""}function ubi_check_cpu(){if(navigator.cpuClass)return"cpu="+escape(navigator.cpuClass);return""}function ubi_check_cookie(){var a=navigator.cookieEnabled?true:false;if(typeof navigator.cookieEnabled=="undefined"&&!a){document.cookie="ubitestcookie";a=document.cookie.indexOf("ubitestcookie")!=-1?true:false}return"cookie="+a}function ubi_check_flash(){if(navigator.plugins&&navigator.plugins.length){var a=navigator.plugins["Shockwave Flash"];if(a&&a.description)return"flash="+a.description.charAt(a.description.indexOf(".")-1);if(navigator.plugins["Shockwave Flash 2.0"])return"flash=2"}for(var b=12;b>0;b--){try{var a=new ActiveXObject("ShockwaveFlash.ShockwaveFlash."+b);return"flash="+b}catch(c){}}return""}function ubi_check_silverlight(){var a="";var b=null;try{var c=null;if(window.navigator.userAgent.indexOf("MSIE")>=0){c=new ActiveXObject("AgControl.AgControl")}else{if(navigator.plugins["Silverlight Plug-In"]){b=document.createElement("div");document.body.appendChild(b);b.innerHTML='<embed type="application/x-silverlight" src="data:," />';c=b.childNodes[0]}}if(c){if(c.isVersionSupported("1.1")){a="1.1"}else if(c.isVersionSupported("1.0")){a="1.0"}}}catch(d){}if(b){document.body.removeChild(b)}if(a!="")return"silverlight="+a;return""}function readCookie(a){var b=a+"=";var c=document.cookie.split(";");for(var d=0;d<c.length;d++){var e=c[d];while(e.charAt(0)==" ")e=e.substring(1,e.length);if(e.indexOf(b)==0)return e.substring(b.length,e.length)}return null}function ubi_get_url(a,b){var c=readCookie("ubisession");var d="";if(!c){d="&sattribute="+escape(ubi_check_screen())+escape("&"+ubi_check_language())+escape("&"+ubi_check_cookie())+escape("&"+ubi_check_flash())+escape("&"+ubi_check_cpu())+escape("&"+ubi_check_silverlight())+escape("&js=true")}if(!ubi_umon_domain){ubi_umon_domain="uts.ubi.com"}if(ubi_umon_domain==""){ubi_umon_domain="uts.ubi.com"}if(!ubi_umon_protocol){ubi_umon_protocol="http"}if(ubi_umon_protocol==""){ubi_umon_protocol="http"}var e=ubi_umon_protocol+"://"+ubi_umon_domain+"/stats.ashx";var f=e+"?system="+ubi_system+"&tag="+escape(a)+"&ref="+ubi_check_referer()+"&value="+b+d+"&version=3.0"+"&cacheid="+Math.floor(Math.random()*1e5);if(ubi_iid){f+="&iid="+ubi_iid}return f}function ubi_do_WT_stat(a,b,c,d){var e=window;if(!a){a=e.ubi_tag}if(!b){b=e.ubi_system}if(!b){b="webtrack"}if(!c){if(!e.ubi_actionvalue){c=""}else{c=e.ubi_actionvalue}}if(a&&b){var f=ubi_get_url(a,c);if(d){var g=new Image;g.src=f}else{document.write("<div style='display:none;'><img src='"+f+"' alt='' /></div>")}}}function ubi_do_WT_stat_silent(a,b,c){if(!c){ubi_do_WT_stat(a,b,"",true)}else{ubi_do_WT_stat(a,b,c,true)}}var ubi_system="WEB";var ubi_umon_domain;var ubi_umon_protocol;var ubi_iid;
function hide_gsawatermark(){if(document.getElementById("q").value=="SEARCH"){e=document.getElementById("q");e.value=""}}function show_gsawatermark(){if(document.getElementById("q").value==""){e=document.getElementById("q");e.value="SEARCH"}}function trim(a,b){return ltrim(rtrim(a,b),b)}function ltrim(a,b){b=b||"\\s";return a.replace(new RegExp("^["+b+"]+","g"),"")}function rtrim(a,b){b=b||"\\s";return a.replace(new RegExp("["+b+"]+$","g"),"")}function goGSASearch(){var a=trim(document.getElementById("q").value);if(a=="SEARCH"||a==""){document.getElementById("q").value="";document.getElementById("q").focus();return false}else{return true}}function Interact(a,b){if(!isIE6)return;if(b==1){a.className="submit over"}else{a.className="submit"}}var isIE6=false;if(/MSIE (\d+\.\d+);/.test(navigator.userAgent)){var ieversion=new Number(RegExp.$1);isIE6=ieversion==6};
